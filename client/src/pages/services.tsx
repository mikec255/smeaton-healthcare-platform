...
export default function Services() {
  return (
    <div data-testid="services-page">
      <Seo
        title="Our Care Services — Devon & Cornwall"
        description="Explore the full range of home care services from Smeaton Healthcare — short visits, live-in care, 24/7 support, respite and more across Devon and Cornwall."
        path="/services"
      />
      <Ticker />
      ...
      <section style={{ backgroundColor: CREAM }}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-8 pb-8 sm:pt-14 sm:pb-12">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-1 tracking-tight" style={{ color: BLUE }}>Care built around</h1>
          <div className="mb-4" style={{ ...SCRIPT, fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", color: PINK }}>you, not a rota.</div>
          <p className="text-gray-500 text-base max-w-2xl leading-relaxed" data-testid="services-hero-subtitle">
          ...
          {SERVICES.map((s) => {
            ...
            return (
              <Link
                key={s.href}
                href={s.href}
                className="group flex items-center gap-5 sm:gap-8 py-5 px-5 sm:px-8 rounded-2xl transition-all hover:shadow-md border-2 border-gray-100 hover:border-gray-200 bg-white"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base sm:text-lg font-extrabold mb-1 tracking-tight group-hover:opacity-80 transition-opacity" style={{ color: NAVY }}>{s.title}</h3>
                ...
                <div className="hidden sm:flex items-center gap-4 shrink-0">
                  ...
                  <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" style={{ color: s.color }} />
                </div>
                <ArrowRight size={15} className="sm:hidden shrink-0 transition-transform group-hover:translate-x-1" style={{ color: s.color }} />
              </Link>
            );
          })}
        </div>
